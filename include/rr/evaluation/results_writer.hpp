...
// `timing` subsection of summary.json is byte-identical across two runs with the same seed. All
// floating-point output uses fixed precision under the classic locale so formatting never depends
// on the ambient locale.
class ResultsWriter {
  public:
